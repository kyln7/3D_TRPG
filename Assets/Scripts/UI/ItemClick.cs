...
public class ItemClick : MonoBehaviour
{
    ...
    public void LoadList()
    {
        ...
        {
            GameObject newItem = Instantiate(newObj);
            newItem.transform.SetParent(GameObject.Find("ItemsMenu").transform);
            Text newItemText = GameObject.Find("itemText").GetComponent<Text>();
            newItemText.text = item.ItemName;
            newItemText.name = "text";
            newItem.name = item.ItemName + "1";
            newItem.transform.localPosition = new Vector3(-237, yPosition, 0);
            yPosition -= 40;
        }
    }
}
